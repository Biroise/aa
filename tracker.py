...
def latlon2trk (lat, lon) :
	x = 90*(aa.cos(lat)*aa.cos(lon) + 1)
	y = 90*(1 - aa.cos(lat)*aa.sin(lon))
	return x, y
...
class File :
	...
	def __init__(self, trk_file_path, no_mountains = True, year=None, lonlat=False) :
		if no_mountains :
			...
			elevation = pickle.load(open('/media/AcuerdaTe/cyclones/orogr/alts.p'))
			#altLats = pickle.load('/media/AcuerdaTe/cyclones/orogr/lats.p')
			#altLons = pickle.load('/media/AcuerdaTe/cyclones/orogr/lons.p')
		...
		while time_stamp != '' :
			...
			if step_nbr*time_step > 24*2 :
				if gc_dist(
						...
						# 1 million meters = 1000 km minimal propagation
						if no_mountains :
							x, y = latlon2trk(
									cy.lats[cy.prssrs.argmin()],
									cy.lons[cy.prssrs.argmin()])
							"""
							if elevation(
							...
							"""
							#print x, y, elevation[round(x), round(y)]
							#if elevation[round(x), round(y)] < 1500 :
							if elevation[round(x), round(y)] < 1000 :
								self.list.append(cy)
	...
	def plot_all (self, basemap = None, color = 'red') :
		if basemap == None :
		...
		for cy in self :
			cy.plot(basemap = basemap, color = color)
...
def load_raw(year, hemisphere='NH') :
	return File('/media/AcuerdaTe/cyclones/' + hemisphere + '/slp.'
			+ str(year) + '.' + hemisphere + '.trk')
		
def load(year, hemisphere='NH') :
	import os
	filePath = '/media/AcuerdaTe/cyclones/'+hemisphere+'/'
